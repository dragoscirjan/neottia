...
Neottia ships four stdio servers. Start each process in the project directory whose `.neottia/config.yml` and canonical files it should use.
...
| Searchable  | `pnpm dlx @neottia/searchable-mcp`  |     5 |
...
Replace the server name and package for Issues, Design Docs, or Searchable. Keep the client process working directory at the project root.
...
All four servers advertise `{tools: {}}` and implement `tools/list` and `tools/call`. They expose no MCP resources or prompts. Keep logs off stdout because stdout carries MCP frames. Stop the client cleanly so it closes the server and its cached domain context.

The servers are non-interactive. A stale `prompt` policy rebuilds; `fail` still fails. See the [Memory](/mcp/memory), [Issues](/mcp/issues), [Design Docs](/mcp/design-docs), and [Searchable](/mcp/searchable) server pages.
