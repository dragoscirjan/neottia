...
Register `issueConfigContribution` in a multi-module registry and pass the immutable shard to `new IssueStore(snapshot.get(issueConfigContribution), cwd)`. Direct typed `IssueConfig` values remain supported. `loadIssueConfig()` is the synchronous standalone compatibility wrapper; it still accepts the deprecated `skills.issues` path, Issues-only file/path variables, all `NEOTTIA_ISSUES_*` value bindings, and explicit overrides. Resolution order is defaults, global file, project file, selected global/project profile, environment, then explicit overrides. See the [configuration guide](../../docs/issues/configuration) for defaults, limits, paths, environment variables, and migration.
