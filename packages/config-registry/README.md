...
`officialConfigContributions` exposes the immutable contribution list, and `officialConfigRegistry` exposes its validated registry. Root files remain strict. Unknown keys fail validation, while every shard published in the [unified configuration guide](../../docs/configuration.md) can coexist in one file. The registry includes compile-time provider connections under `connections.forges` for GitHub, GitLab, Gitea, Forgejo, Bitbucket, Jira, and Confluence. It also includes role assignment maps under `agents.sdlc` keyed by adapter ID, with documented `pi` and `opencode` keys and validated maps for additional adapters such as `claude-code`.
